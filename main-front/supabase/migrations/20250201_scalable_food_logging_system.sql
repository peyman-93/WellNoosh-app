-- ================================================================================================
-- WellNoosh Scalable Food Logging System
-- Migration: 20250201_scalable_food_logging_system.sql
...
CREATE TABLE user_food_logs (
  ...
  -- Reference to planned meal (nullable for additional food)
...
SELECT create_monthly_partition(CURRENT_DATE + (i || ' months')::INTERVAL)
...
BEGIN
  ...
  FOR i IN 0..2 LOOP
    result_text := result_text || chr(10) || create_monthly_partition(CURRENT_DATE + (i || ' months')::INTERVAL);
  END LOOP;
...
ALTER TABLE user_food_logs_archive DROP CONSTRAINT user_food_logs_log_date_check;
...
DO $$
BEGIN
  RAISE NOTICE 'Scalable Food Logging System migration completed successfully!';
